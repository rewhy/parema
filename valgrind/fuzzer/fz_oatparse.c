...
static Addr	fz_mem_start = 0;
static Addr fz_mem_end		= 0;

struct DexFile *pBootDex = NULL;

extern UInt fz_sink_method_index;

extern Addr string_equals_entry_addr;
extern Addr log_println_native_entry_addr;
extern Addr lang_exception_init_addr;
extern Addr native_poll_once_addr;
extern Addr runtime_get_runtime_addr;

Bool fz_is_mem_range(Addr a, UInt len) {
	if( a + len < fz_mem_start)
		return False;
	if( a > fz_mem_end )
		return False;
	return True;
...
//extern VgHashTable *fz_frame_mth_list;
VgHashTable* fz_frame_mth_list = NULL;

static void* add_method(HChar *clazz, HChar* mth, HChar* shorty, Addr codeAddr, SizeT codeSize, Int index, Int accessFlags, Bool isBoot)
{
	if(is_taint_white(codeAddr))
		return NULL;
	if(isBoot == False) {
		if (VG_(memcmp)("access", mth, 6) == 0)
			return NULL;
	} else {
		if(index == 13334) {
			if(VG_(strcmp)("equals", mth) == 0) {
				string_equals_entry_addr = codeAddr; 
				VG_(printf)("String.equals()'s Addr is 0x%08x\n", string_equals_entry_addr);
			}
		} else if(index == 58705) {
			if(VG_(strcmp)("println_native", mth) == 0) {
				log_println_native_entry_addr = codeAddr; 
				VG_(printf)("Log.println_native()'s Addr is 0x%08x\n", log_println_native_entry_addr);
			}
		} else if(index == 12692) {
			lang_exception_init_addr = codeAddr;
		} else if(index == 43834) {
			if(VG_(strcmp)("nativePollOnce", mth) == 0) {
				native_poll_once_addr = codeAddr;
				VG_(printf)("MessageQueue.nativePollOnce()'s Addr is 0x%08x\n", native_poll_once_addr);
			}
		} else if(index == 13132) {
			if(VG_(strcmp)("getRuntime", mth) == 0) {
				runtime_get_runtime_addr = codeAddr;
				VG_(printf)("Runtime.getRuntime()'s Addr is 0x%08x\n", native_poll_once_addr);
			}
		}
	}
	...
	if(mth_node) {
		...
		if(isBoot) {
			mth_node->type |= TYPE_BOOT;
		} else {
			mth_node->type |= TYPE_BASE;
		}
		...
		if(mth_list == NULL) {
			...
			VG_(HT_add_node)(fz_frame_mth_list, mth_list);
		}
		if(mth_list->num < MAX_METHOD_NUM) {
			mth_list->mthNodes[mth_list->num] = (Addr)mth_node;
			mth_list->num++;
		} else {
			VG_(printf)("Warning: List for addr 0x%08x (%s %s) is already full\n", codeAddr, clazz, mth);
		}
#if 0
		VG_(printf)("1 Add Method(%d): 0x%08x-0x%08x %s %s() %s source=%s\n", index, codeAddr, codeAddr+codeSize-1, mth_node->clazz, 
				mth_node->method, mth_node->shorty, mth_node->type&TYPE_SOURCE ? "True" : "False" );
#endif
		if (index == fz_sink_method_index) {
			fz_mem_start	= codeAddr & ~0x1;
			fz_mem_end		= codeAddr + codeSize - 1;
...
MthList* query_method_list(Addr codeAddr) {
	if(fz_frame_mth_list == NULL)
		return NULL;
	MthList *mth_list = VG_(HT_lookup)( fz_frame_mth_list, codeAddr);
	if(mth_list)
		return mth_list;
	return NULL;
}
...
Bool query_method(Addr codeAddr, HChar **clazz, HChar **mth, HChar **shorty, Int *accFlags)
{
	...
	MthNode *mth_node = VG_(HT_lookup)( fz_frame_mth_list, codeAddr);
	if(mth_node) {
		if(clazz)
...
{
	MthNode *mth_node = VG_(HT_remove)( fz_frame_mth_list, codeAddr);
	if( NULL == mth_node )
		return;
	VG_(free)( mth_node );
	mth_node = NULL;
...
Bool oatDexClassParse(Addr oatdata, 
	...
	if(VG_(memcmp)("Landroid/support", className, 16) == 0)
		return False;
#if 0
	/* prInt debug info */
	...
	for (i=0;i<direct_methods_size;i++) {
		...
		if(native_code_offset > 0)
		{
			...
			add_method(className, str, shorty, oatdata+native_code_offset, oat_mth_header->codeSize, key, method_access_flags, isBoot);
		}
...
Bool oatDexFileParse(Addr oatdata, 
		...
		Addr dexBuf, UInt size, Bool isBoot) {
	UInt j = 0;
	...
	if(fz_frame_mth_list == NULL) {
		fz_frame_mth_list = VG_(HT_construct)( "fz_frame_mth_list" );
	}
	...
	for(j = 0; j < dh->classDefsSize; j++) {
		...
		oatDexClassParse(oatdata, dexBuf, offset, oat_class_header, pDex, isBoot);
	}
	OAT_LOGI("Finished.\n");
	return True;
}
...
Bool oatDexParse(Addr oatdata, UInt oatdata_size,
		Addr oatexec, UInt oatexec_size, Bool isBoot) {
	UInt i = 0;
	...
	for(i = 0; i < oh->dexFileCount; i++) {
		...
		OAT_LOGI("\tDex file size: %d\n\n", dh->fileSize);
		oatDexFileParse(oatdata, classes_offsets, (Addr)dh, oatdata_size, isBoot);
