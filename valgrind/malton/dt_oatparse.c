...
//extern VgHashTable *DT_(frame_mth_list);
VgHashTable* DT_(frame_mth_list) = NULL;
...
static void* add_method(HChar *clazz, HChar* mth, HChar* shorty, Addr codeAddr, SizeT codeSize, Int index, Int accessFlags)
{
	...
	if(mth_node) {
		...
		if(mth_list == NULL) {
			...
			VG_(HT_add_node)(DT_(frame_mth_list), mth_list);
		}
...
MthList* query_method_list(Addr codeAddr) {
	if(DT_(frame_mth_list) == NULL)
		return NULL;
	MthList *mth_list = VG_(HT_lookup)( DT_(frame_mth_list), codeAddr);
	if(mth_list)
		return mth_list;
	return NULL;
}
...
Bool query_method(Addr codeAddr, HChar **clazz, HChar **mth, HChar **shorty, Int *accFlags)
{
	...
	MthNode *mth_node = VG_(HT_lookup)( DT_(frame_mth_list), codeAddr);
	if(mth_node) {
		if(clazz)
...
{
	MthNode *mth_node = VG_(HT_remove)( DT_(frame_mth_list), codeAddr);
	if( NULL == mth_node )
		return;
	VG_(free)( mth_node );
	mth_node = NULL;
...
Bool oatDexFileParse(Addr oatdata, 
	...
	if(DT_(frame_mth_list) == NULL) {
		DT_(frame_mth_list) = VG_(HT_construct)( "DT_(frame_mth_list)" );
	}
