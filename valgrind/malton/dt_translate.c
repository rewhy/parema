		...
		IRAtom* atom, IRAtom* vatom, IRExpr *guard )
{
	...
	if( atom && clone && !DT_(clo_critical_ins_only) ){
		...
		IRAtom* addr, UInt bias, IRAtom* guard )
{
	...
	if (end == Iend_LE) {
		switch (ty) {
			case Ity_V256: helper = &DT_(helperc_LOADV256le);
										 hname = "DT_(helperc_LOADV256le)";
										 ret_via_outparam = True;
										 break;
			case Ity_V128: helper = &DT_(helperc_LOADV128le);
										 hname = "DT_(helperc_LOADV128le)";
										 ret_via_outparam = True;
										 break;
			case Ity_I64:  helper = &DT_(helperc_LOADV64le);
										 hname = "DT_(helperc_LOADV64le)";
										 break;
			case Ity_I32:  helper = &DT_(helperc_LOADV32le);
										 hname = "DT_(helperc_LOADV32le)";
										 break;
			case Ity_I16:  helper = &DT_(helperc_LOADV16le);
										 hname = "DT_(helperc_LOADV16le)";
										 break;
			case Ity_I8:   helper = &DT_(helperc_LOADV8);
										 hname = "DT_(helperc_LOADV8)";
										 break;
			default:       ppIRType(ty);
		...
		switch (ty) {
			case Ity_V256: helper = &DT_(helperc_LOADV256be);
										 hname = "DT_(helperc_LOADV256be)";
										 ret_via_outparam = True;
										 break;
			case Ity_V128: helper = &DT_(helperc_LOADV128be);
										 hname = "DT_(helperc_LOADV128be)";
										 ret_via_outparam = True;
										 break;
			case Ity_I64:  helper = &DT_(helperc_LOADV64be);
										 hname = "DT_(helperc_LOADV64be)";
										 break;
			case Ity_I32:  helper = &DT_(helperc_LOADV32be);
										 hname = "DT_(helperc_LOADV32be)";
										 break;
			case Ity_I16:  helper = &DT_(helperc_LOADV16be);
										 hname = "DT_(helperc_LOADV16be)";
										 break;
			case Ity_I8:   helper = &DT_(helperc_LOADV8);
										 hname = "DT_(helperc_LOADV8)";
										 break;
		...
		IRAtom* guard )
{
	...
	if (end == Iend_LE) {
		switch (ty) {
			...
			case Ity_I64: helper = &DT_(helperc_STOREV64le);
										hname = "DT_(helperc_STOREV64le)";
										break;
			case Ity_I32: helper = &DT_(helperc_STOREV32le);
										hname = "DT_(helperc_STOREV32le)";
										break;
			case Ity_I16: helper = &DT_(helperc_STOREV16le);
										hname = "DT_(helperc_STOREV16le)";
										break;
			case Ity_I8:  helper = &DT_(helperc_STOREV8);
										hname = "DT_(helperc_STOREV8)";
										break;
			default:      VG_(tool_panic)("dt_translate.c:do_shadow_Store(LE)");
		}
	} else {
		switch (ty) {
			case Ity_V128: /* we'll use the helper twice */
			case Ity_I64: helper = &DT_(helperc_STOREV64be);
										hname = "DT_(helperc_STOREV64be)";
										break;
			case Ity_I32: helper = &DT_(helperc_STOREV32be);
										hname = "DT_(helperc_STOREV32be)";
										break;
			case Ity_I16: helper = &DT_(helperc_STOREV16be);
										hname = "DT_(helperc_STOREV16be)";
										break;
			case Ity_I8:  helper = &DT_(helperc_STOREV8);
										hname = "DT_(helperc_STOREV8)";
										break;
...
//           VG_(fnptr_to_fnentry)( &DT_(helperc_MAKE_STACK_UNINIT) ),
...
static void do_shadow_CAS_single ( MCEnv* mce, IRCAS* cas )
{
	...
	//Int    elemSzB;
	IRType elemTy;
	//   Bool   otrak = DT_(clo_dt_level) >= 3; /* a shorthand */
...
static void do_shadow_CAS_double ( MCEnv* mce, IRCAS* cas )
{
	...
	IRType elemTy;
	//   Bool   otrak = DT_(clo_tnt_level) >= 3; /* a shorthand */
...
passed from DT_(instrument) to its subroutines to 
...
IRDirty* create_dirty_CCALL( MCEnv* mce, IRStmt *clone, IRTemp tmp,
	...
	if ( mce->hWordTy == Ity_I32 ) {
		if ( VG_(strcmp)(cee->name, "x86g_calculate_condition") == 0 ) {
			fn    = &DT_(h32_x86g_calculate_condition);
			nm    = "DT_(h32_x86g_calculate_condition)";
		} else {
			fn    = &DT_(h32_ccall);
			nm    = "DT_(h32_ccall)";
		}
		...
		if ( VG_(strcmp)(cee->name, "amd64g_calculate_condition") == 0 ) {
			fn    = &DT_(h64_amd64g_calculate_condition);
			nm    = "DT_(h64_amd64g_calculate_condition)";
		} else {
			fn    = &DT_(h64_ccall);
			nm    = "DT_(h64_ccall)";
		}
...
# endif
	...
	if(mNode == NULL || i == mList->num)
		return;
	/*mth_top_stack(tid, &last_lr, NULL);
		if(lr == last_lr)
		return;
		if((mNode->codeAddr&0xfffffffe) != (pc&0xfffffffe)) {
		ART_LOGW("Pc (0x%08x) & codeAddr (0x%08x) missmatch!!!\n", pc, mNode->codeAddr);
		mNode = query_method_node(pc&0xfffffffe);
		}
		if(!mNode)
		return;
		*/
		...
		IRType	gWordTy, IRType	hWordTy )
{
	...
	mce.tmpMap = VG_(newXA)( VG_(malloc), "mc.DT_(instrument).1", VG_(free),
			sizeof(TempMapEnt));
	...
	for (/* use existing i */; i < sbIn->stmts_used; i++) {
		...
		switch (st->tag) {
			...
			default:
				...
				ppIRStmt(st);
				VG_(printf)("\n");
				VG_(tool_panic)("dt_translate.c: DT_(instrument): unhandled IRStmt");
		} /* switch (st->tag) */
