...
Bool BG_(is_trace_meminfo)  	= False;
/* End */
...
{
	int trace_load_stmt		= -1;
	int trace_store_stmt	= -1;
	if			VG_BOOL_CLO(arg, "--trace-load",		BG_(is_instrument_load)) {}
	else if VG_BOOL_CLO(arg, "--trace_store",		BG_(is_instrument_store)) {}
	else if VG_BOOL_CLO(arg, "--trace_syscall",	BG_(is_trace_syscall)) {}
	else if VG_BOOL_CLO(arg, "--trace_meminfo",	BG_(is_trace_meminfo)) {}
	else 
...
Bool BG_(handle_client_requests) ( ThreadId tid, UWord *arg, UWord *ret) {
		...
		case VG_USERREQ__WRAPPER_GETTIMEOFDAY:
			{
				...
				if(last_ttt == 1) {
					last_ts = tv->tv_sec * 1000000ULL + tv->tv_usec;
				} else {
					tv->tv_sec  = (last_ts+(ULong)last_ttt) / 1000000;
					tv->tv_usec = (last_ts+(ULong)last_ttt) % 1000000;
				}
		...
		case VG_USERREQ__WRAPPER_DEXFILEPARSE:
			{
				...
				if(pf) {
					...
					addDexFileList(pf);
					meetDexFile(pf, begin_addr, len, 1);
				}
				break;
			}
		case VG_USERREQ__WRAPPER_DVMDEFINECLASS_PRE:
			{
				...
				if( isInDexFileList(pdf) == NULL)
					break;
				BG_LOGI("[0]DVMWRAP(%d):dvmDefineClass() pDexFile=0x%08x map: 0x%08x-0x%08x pHeader=0x%08x(0x%08x) pOptHeader=0x%08x\n", 
		...
		case VG_USERREQ__WRAPPER_DVMDEFINECLASS:
			{
				...
				if(poj) {
					...
					BG_LOGI("[1]DVMWRAP(%d):dvmDefineClass() pDexFile=0x%08x map: 0x%08x-0x%08x pHeader=0x%08x(0x%08x) pOptHeader=0x%08x %s res=NULL\n", 
							tid, (Addr)pdf, mm.addr, mm.addr+mm.length, (Addr)pdh, (Addr)pdf->pHeader, (Addr)pdf->pOptHeader, des);
				}
				//if(True/*is_parsed((Addr)pdd) == False*/) {
				if( isInDexFileList(pdf) == False ) {
#ifndef ONLY_DUMP
#ifndef M_PERFORMANCE
					processDexFile(pdf);
...
#ifdef	IJIAMI_1603
				if(poj) {
					if(poj->descriptor[1]=='h' && poj->descriptor[2]=='i' && poj->descriptor[3]=='o' && poj->descriptor[4]=='f') {
	...
	static void bg_pre_clo_init(void)
	{
		...
		VG_(details_copyright_author)(
				"Copyright (C) 2002-2017, and GNU GPL'd, by Rewhy");
