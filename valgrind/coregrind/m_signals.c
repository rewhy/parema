...
Bool VG_(extend_stack)(ThreadId tid, Addr addr)
{
	...
	if (seg->kind == SkAnonC)  {
		...
		//VG_(dmsg)("Rewhy: base=%p, seg=%p-%p\n", addr, seg->start, seg->end);
		return True;
	}
	...
	VG_(dmsg)("Rewhy: Extending a stack base 0x%lx (next_seg=0x%lx-0x%lx) down by %lu\n", 
			seg_next->start, seg_next->start, seg_next->end,  udelta);
	Bool overflow;
...
static Bool extend_stack_if_appropriate(ThreadId tid, vki_siginfo_t* info)
{
	...
	if (VG_(clo_trace_signals)) {
		if (seg == NULL) {
			...
			VG_(dmsg)("SIGSEGV: si_code=%d faultaddr=%#lx tid=%u ESP=%#lx "
					"seg=%#lx-%#lx kind=0x%02x mode=0x%d seg_next=%p next_kind=%d fault_mask=%p\n",
					info->si_code, fault, tid, esp, seg->start, seg->end, seg->kind, seg->smode,
					(Addr)seg_next, seg_next->kind, fault_mask(esp - VG_STACK_REDZONE_SZB));
		}
	}
#if 1 // Rewhy
	if (info->si_code == VKI_SEGV_MAPERR
		...
		/* If the fault address is above esp but below the current known
			 ...
			 */
		Addr base = 0;
		if(fault >= fault_mask(esp - VG_STACK_REDZONE_SZB)) {
			base = VG_PGROUNDDN(esp - VG_STACK_REDZONE_SZB) - 15 * VKI_PAGE_SIZE;
		}	else {
			base = VG_PGROUNDDN(fault - VG_STACK_REDZONE_SZB);// - 63 * VKI_PAGE_SIZE;
		}
		VG_(dmsg)("Rewhy: Try to extend stack base from %#lx to %#lx\n", base, VG_PGROUNDDN(fault)); // Rewhy
		if (VG_(am_addr_is_in_extensible_client_stack)(base) && VG_(extend_stack)(tid, base)) {
			if (VG_(clo_trace_signals))
				VG_(dmsg)("       -> extended stack base to %#lx\n", base);
			return True;
		} else {
	...
	if (info->si_code == VKI_SEGV_MAPERR
		...
		/* If the fault address is above esp but below the current known
			 ...
			 */
		Addr base = VG_PGROUNDDN(esp - VG_STACK_REDZONE_SZB);
		VG_(dmsg)("Rewhy: Try to extended stack base from %#lx to %#lx\n", base, VG_PGROUNDDN(fault)); // Rewhy
		if (VG_(am_addr_is_in_extensible_client_stack)(base) && VG_(extend_stack)(tid, base)) {
			if (VG_(clo_trace_signals)) {
				VG_(dmsg)("       -> extended stack base to %#lx\n",	VG_PGROUNDDN(fault));
			}
		...
		Int sigNo, vki_siginfo_t *info, struct vki_ucontext *uc )
{
	...
	if (extend_stack_if_appropriate(tid, info)) {
		/* Stack extension occurred, so we don't need to do anything else; upon
			 ...
			 instruction. */
			if (VG_(clo_trace_signals))
				VG_(dmsg)("       -> Finished sync signalhaneler from kernel.\n"); // Rewhy
	} else {
		...
		vki_siginfo_t *info, struct vki_ucontext *uc )
{
	...
	//if (0) 
	if (VG_(clo_trace_signals)) // Rewhy
