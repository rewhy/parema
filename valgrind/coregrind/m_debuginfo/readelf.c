	...
	vg_assert(di->soname == NULL);
	{
		...
		for (i = 0; i < phdr_mnent; i++) {
			...
			if (a_phdr.p_type == PT_LOAD) {
				TRACE_SYMTAB("PT_LOAD[%ld]: p_vaddr 0x%#lx (prev 0x%#lx)\n",
						i, (UWord)a_phdr.p_vaddr, (UWord)prev_svma);
				TRACE_SYMTAB("PT_LOAD[%ld]: p_offset 0x%lx, p_filesz 0x%lx, perms %c%c%c\n",
				...
				if (a_phdr.p_vaddr < prev_svma) {
					ML_(symerr)(di, True,	"ELF Program Headers are not in ascending order");
					goto out;
				}
				...
				if (a_phdr.p_memsz > 0) {
					...
					for (j = 0; j < VG_(sizeXA)(di->fsm.maps); j++) {
						...
						TRACE_SYMTAB("MAP: 0x%#lx 0x%#lx 0x%#lx %lx %c%c%c\n", 
								j, map->avma, map->foff, map->size, 
								map->ro & PF_R ? 'r' : '-',
								map->rw & PF_W ? 'w' : '-', 
								map->rx & PF_X ? 'x' : '-');
						//if (   (map->rx || map->rw)
						if ((map->rx || map->rw || map->ro) /* Changed to this by Rewhy */
							...
							item.bias       = map->avma - map->foff + a_phdr.p_offset - a_phdr.p_vaddr;
							if (map->rw && (a_phdr.p_flags & (PF_R | PF_W)) == (PF_R | PF_W)) {
								item.exec = False;
							...
							if (map->rx && (a_phdr.p_flags & (PF_R | PF_X))	== (PF_R | PF_X)) {
								item.exec = True;
								VG_(addToXA)(svma_ranges, &item);
								TRACE_SYMTAB(
							...
							/* Added by Rewhy */
							if(map->ro && ((a_phdr.p_flags & PF_R) == PF_R)) {
								item.exec = False;
								VG_(addToXA)(svma_ranges, &item);
								TRACE_SYMTAB(
					...
					if (!loaded) {
						ML_(symerr)(di, False, "ELF section outside all mapped regions");
			...
			if (a_phdr.p_type == PT_DYNAMIC && di->soname == NULL) {
				...
				if (stroff != -1 && strtab_mioff != DiOffT_INVALID) {
					di->soname = ML_(img_strdup)(mimg, "di.redi.1", strtab_mioff + stroff);
...
#     define BAD(_secname)                                 \
		...
		if (0 == VG_(strcmp)(name, ".rodata")) {
			...
			if ((inrx || inrw) && !di->rodata_present) { // Changed to here by Rewhy
		...
		/* Accept .dynsym where mapped as rw (data) Rewhy */
		if (0 == VG_(strcmp)(name, ".dynsym")) {
			//vg_assert(!di->dynsym_present);
			if ((inrw || inrx) && !di->dynsym_present) {
				di->dynsym_present = True;
				if(inrw)
					di->dynsym_avma = svma + inrw->bias;
				else
					di->dynsym_avma = svma + inrx->bias;
				di->dynsym_size = size;
				TRACE_SYMTAB("acquiring .dynsym avma = %#lx\n", di->dynsym_avma);
			} else if (size != 0) {
				BAD(".dynsym");
			} 
		}   
...
#        endif
				...
				/* 1) Output tracing logs is added;
				 * 2) Reading .rel.plt table is added;
				 * Modified by Rewhy
				 */
				TRACE_SYMTAB("symtab img: %#lx sz=%d sym=%d %d\n",
						(Addr)symtab_escn.img,
						symtab_escn.szB, sizeof(ElfXX_Sym),
						symtab_escn.szB % sizeof(ElfXX_Sym));
				...
				TRACE_SYMTAB("dynsym img: %#lx sz=%d sym=%d %d\n",
						(Addr)dynsym_escn.img,
						dynsym_escn.szB, sizeof(ElfXX_Sym),
						dynsym_escn.szB % sizeof(ElfXX_Sym));
			...
			if (di->varinfo) {
				for (j = 0; j < VG_(sizeXA)(di->varinfo); j++) {
					...
					while (True) {
						...
						vg_assert(w >= 0);
						if (1) VG_(printf)("range %#lx %#lx %ld\n",
