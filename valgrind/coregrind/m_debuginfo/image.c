...
static Bool ensure_valid(const DiImage* img, DiOffT offset, SizeT size, // Rewhy
                         const HChar* caller)
{
   if (LIKELY(ML_(img_valid)(img, offset, size)))
      return True;
   else {
    //  ensure_valid_failed(img, offset, size, caller); // Rewhy
		return False;
	 }
}
                  ...
                  DiImage* img, DiOffT offset, SizeT size)
{
   vg_assert(img);
   vg_assert(size > 0);
   if(!ensure_valid(img, offset, size, "ML_(img_get)")) {// Rewhy
		 VG_(memset)((Addr)dst, 0, size);
		 return;
	 }
   SizeT i;
   for (i = 0; i < size; i++) {
