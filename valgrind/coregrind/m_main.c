...
#if DBG_REWHY
	VG_(printf)("REWHY: Initialise the tool part 2 (post_clo_init)\n");
#endif
...
#if DBG_REWHY
	VG_(printf)("REWHY: Initialise TT/TC\n");
#endif
	VG_(init_tt_tc)();
...
#if DBG_REWHY
	VG_(printf)("REWHY: Initialise redirects.\n");
#endif
...
#if DBG_REWHY
		VG_(printf)("REWHY: Init preopened fds.\n");
#endif
...
#if DBG_DEBUG
	VG_(printf)("Load initial debug info.");
#endif
...
#if DBG_REWHY
			VG_(printf)("REWHY: n_seg_starts %d %d\n", n_seg_starts, i);
#endif
			anu.ull = VG_(di_notify_mmap)( seg_starts[i], True/*allow_SkFileV*/, -1/*Don't use_fd*/);
			/* anu.ull holds the debuginfo handle returned by di_notify_mmap, if any. */
			if (anu.ull > 0) {
				anu.a = seg_starts[i];
...
#  endif
...
#if DBG_REWHY
		VG_(printf)("REWHY: transfer ownership V -> C of 0x%llx .. 0x%llx\n",
...
#if DBG_REWHY
	VG_(printf)("\nREWHY: Initialise scheduler (phase 1).\n\n");
#endif
	...
	//--------------------------------------------------------------
#if DBG_REWHY
	VG_(printf)("\nREWHY: Tell tool about initial permissions.\n\n");
#endif
		...
		for (i = 0; i < n_seg_starts; i++) {
			...
			vg_assert(seg->start == seg_starts[i]);
			{
				VG_(debugLog)(2, "main", 
						...
						seg->hasX ? 'x' : '-' );
#if DBG_REWHY
				VG_(printf)("REWHY: tell tool about %010lx-%010lx %c%c%c\n",
						seg->start, seg->end,
						seg->hasR ? 'r' : '-',
						seg->hasW ? 'w' : '-',
						seg->hasX ? 'x' : '-' );
...
#if DBG_REWHY
			VG_(printf)("REWHY: mark stack inaccessible %010lx-%010lx\n",
					seg->start, 
					the_iifii.initial_client_SP-1 - VG_STACK_REDZONE_SZB);
#endif
		}
...
#    endif
	...
	//--------------------------------------------------------------
#if DBG_REWHY
	VG_(printf)("\nREWHY: Initialise scheduler (phase 2).\n\n");
#endif
...
#if DBG_REWHY
	VG_(printf)("REWHY: Initialise signal management 1.\n");
#endif
	/* Check that the kernel-interface signal definitions look sane */
	VG_(vki_do_initial_consistency_checks)();
#if DBG_REWHY
	VG_(printf)("REWHY: Initialise signal management 2.\n");
#endif
	/* .. and go on to use them. */
	VG_(sigstartup_actions)();
#if DBG_REWHY
	VG_(printf)("REWHY: Initialise signal management 3.\n");
#endif
...
#if DBG_REWHY
		VG_(printf)("REWHY: Load suppressions.\n");
#endif
...
#if DBG_REWHY
	VG_(printf)("REWHY: Running thread 1\n");
#endif
