...
Bool check_string_tainted(StringMirror *strObj)
{
	...
	if(strObj && strObj->count_ > 0){
		...
		isTaint = DT_(check_mem_tainted)((Addr)strObj->value_, len);
	}
	return isTaint;
}
...
# if defined(VGPV_arm_linux_android)
	...
	if(mNode->accessFlags & ACC_STATIC) { // Static method
		...
		isTaint = DT_(check_reg_tainted)(1, tid);
		content = NULL;
		...
		switch(type) {
			case 'L':
				...
				if(j < 4) {
					if(r[j] > 0)
						...
						clazz = NULL;
					isTaint = DT_(check_reg_tainted)(j, tid);
					j += 1;
				}	else {
					isTaint = DT_(check_mem_tainted)((Addr)sp, 4);
				...
				if(tmp[0] == '[') {
					...
					isTaint = DT_(check_mem_tainted)(objAddr + sizeof(ObjectMirror) + sizeof(UInt), len);
				} else if(VG_(strcmp)("java.lang.String", tmp) == 0) {
			...
			case 'Z': // Bool
				if( j < 4) {
					...
					bres = (Bool)(*(Int*)sp);
					sp += sizeof(Int);
					isTaint = DT_(check_mem_tainted)(sp, 4);
				}
			...
			case 'J':
				if( j < 3) {
					...
					jres |= r[j];
					isTaint = DT_(check_reg_tainted)(j, tid);
					isTaint |= DT_(check_reg_tainted)(j+1, tid);
					j += 2;
				} else {
					jres = *(Long*)sp;
					sp += sizeof(Long);
					isTaint = DT_(check_mem_tainted)(sp, 8);
				}
...
# if defined(VGPV_arm_linux_android)
	...
	switch(type) {
		case 'L':
			...
			isTaint = DT_(check_reg_tainted)(0, tid);
			if(tmp[0] == '[') {
				len = process_array_arg(tmp, r[0]);
				isTaint = DT_(check_mem_tainted)(r[0] + sizeof(ObjectMirror) + sizeof(UInt), len);
				if(isTaint)
					DT_(make_reg_tainted)(0, tid);
				else
...
#endif
		len = sizeof(StringMirror) + strObj->count_ * sizeof(UShort);
		DT_(check_mem_tainted)((Addr)strObj, len);
		return strObj->count_;
	}	else {
		VG_(printf)("NULL\n");
	}
...
//void DT_(make_reg_tainted) (Int r);
void make_int_tainted(Int reg, Int *addr) {
	if( addr ) {
		DT_(make_mem_tainted)((Addr)addr, 4);
	} else {
		DT_(make_reg_tainted)( reg );
	}
}
void make_int_untainted(Int reg, Int *addr) {
	if( addr ) {
		DT_(make_mem_untainted)((Addr)addr, 4);
	} else {
		DT_(make_reg_untainted)( reg );
	}
	Bool check_int_tainted(Int reg, Int *addr) {
		if( addr ) {
			return DT_(make_mem_untainted)((Addr)addr, 4);
		} else {
			return DT_(make_reg_untainted)( reg );
		}
		return False;
	}
