...
extern UInt fz_method_index;
extern UInt fz_arg_taint_method_index;
extern HChar *fz_arg_taint_method_name;
extern Addr string_equals_return_addr;
//extern UInt fz_res_taint_method_index;
//extern HChar *fz_res_taint_method_index;
...
static
HChar* get_stringobj_dep(Addr addr)
{
	StringMirror *strObj = (StringMirror *)addr;
	Int size = 0, i = 0, j = 0;
	UShort *value = NULL;
...
/*--------------------- For taint source ----------------------------*/
static
UInt set_string_value(StringMirror *strObj, UShort *value_buf, UInt len)
{
	UShort *value = NULL;
	UInt count = len / sizeof(UShort);
	UInt i = 0;
	tl_assert(count  <= strObj->count_);
	if(strObj && count > 0){
		EXP_LOGI("set_string_value() ");
		if(strObj->count_ != count) {
			EXP_LOGI("Dis-match count: %d %d\n", count, strObj->count_);
		}
		strObj->count_ = count;
		for( i = 0; i < count; i++) {
			strObj->value_[i] = value_buf[i];
			VG_(printf)(" 0x%04x", value_buf[i]);
		}
		VG_(printf)("\n");
	}
	return len;
...
static
UInt cmp_string_value(StringMirror *strObj1, StringMirror *strObj2) {
	UShort *value1 = strObj1->value_;
	UShort *value2 = strObj2->value_;
	Int i = 0;
	// EXP_LOGI("Len = %d %d\n", strObj1->count_, strObj2->count_);
	if(strObj1->count_ != strObj2->count_) {
		return 0;
	}
	for(i = 0; i < strObj1->count_; i++) {
		//EXP_LOGI("0x%04x 0x%04x\n", value1[i], value2[i]);
...
static
Int get_string_buf(StringMirror *strObj, UChar* buf)
{
	UInt	 len = 0;
	if(strObj && strObj->count_ > 0){
		len = strObj->count_ * sizeof(Short);
		VG_(memcpy)(buf, strObj->value_, len);
	}
	return len;
}
...
Int make_string_tainted(StringMirror *strObj) 
{
	...
	if(strObj && strObj->count_ > 0){
		value = strObj->value_;
		EXP_LOGI("taint: 0x%08x %d:\n", strObj->value_, strObj->count_);
...
UInt do_taint_source(MthNode *mNode, ThreadId tid, UChar* iBuf)
{
	...
	switch(mNode->shorty[0]) {
		case 'L':
			...
			if(tmp[0] == '[') {
				...
				// TODO: Add taint process
			} else if(VG_(strcmp)("java.lang.String", tmp) == 0) {
				content = get_stringobj_str((StringMirror *)r[0]);
				size = make_string_tainted((StringMirror*)r[0]);
				get_string_buf((StringMirror*)r[0], iBuf);
			}
...
UInt set_taint_source_value(MthNode *mNode, ThreadId tid, UChar *value_buf, UInt len)
{
	if(mNode->shorty == NULL) {
		EXP_LOGI("%s shorty is NULL!!!!\n", mNode->method);
		return;
	}
	if(mNode->shorty[0] == 'V') {
		EXP_LOGI("%s has has no return value!!!!\n", mNode->method);
		return;
	}
	ThreadState *tst	= VG_(get_ThreadState) ( tid );
	VexGuestArchState *arch_state = &tst->arch.vex;
...
# if defined(VGPV_arm_linux_android)
	...
	r1 = arch_state->guest_R1;
#endif
	//EXP_LOGI("set_taint_source_value()\n");
	switch(mNode->shorty[0]) {
		case 'L':
			clazz = (ClassMirror *)r0;
			if((Addr)clazz) {
				clazz = clazz->object_.klass_;
			} else {
				clazz = NULL;
			...
			if(tmp[0] == '[') {
				// TODO: Add taint process
				EXP_LOGI("Un-suport binary type.\n");
			} else if(VG_(strcmp)("java.lang.String", tmp) == 0) {
				HChar* content = get_stringobj_str((StringMirror *)r0);
				EXP_LOGI("Orig_content: %s\n", content);
				size = set_string_value((StringMirror*)r0, (UShort*)value_buf, len);
			}
			break;
		case 'C':
			value = (UInt)(*((UChar*)value_buf));
			setRegValue(8, value);
			break;
		case 'S':
			value = (UInt)(*((UShort*)value_buf));
			setRegValue(8, value);
			break;
		case 'I':
			value = *((UInt*)value_buf);
			setRegValue(8, value);
			break;
		case 'J':
		case 'F':
		case 'D':
			VG_(printf)("Un-suport type of results.\n");
			break;
		case 'V':
		default:
			break;
...
UInt hook_string_equals(ThreadId tid, UInt dst) {
	ThreadState *tst	= VG_(get_ThreadState) ( tid );
	VexGuestArchState *arch_state = &tst->arch.vex;
	ClassMirror *strClazz1 = NULL, *strClazz2 = NULL;
	UWord *r = NULL;
	Addr ret = dst;
	UInt size = 0, size1 = 0, size2 = 0;
	HChar dep[DEP_MAX_LEN] = {0};
	HChar dep1[DEP_MAX_LEN] = {0};
	HChar dep2[DEP_MAX_LEN] = {0};
	HChar tmp1[256] = {0};
	HChar tmp2[256] = {0};
	HChar content1[256] = {0};
	HChar content2[256] = {0};
	Bool  isTainted1 = False, isTainted2 = False;
//	= get_stringobj_str((StringMirror *)r[0]);
#if defined(VGPV_arm_linux_android)
	r = (UWord*)&arch_state->guest_R0;
#endif
	if( r == NULL )
		return ret;
	strClazz1	= (ClassMirror *)r[1];
	strClazz2	= (ClassMirror *)r[2];
	if(strClazz1)
			strClazz1 = strClazz1->object_.klass_;
	if(strClazz2)
			strClazz2 = strClazz2->object_.klass_;
	VG_(strcpy)(tmp1,get_classobject_name(strClazz1));
	VG_(strcpy)(tmp2,get_classobject_name(strClazz2));
	//VG_(printf)("%s  %s\n", tmp1, tmp2);
	if((VG_(strcmp)("java.lang.String", tmp1) == 0)
			&& (VG_(strcmp)("java.lang.String", tmp2) == 0)) {
		StringMirror *sc1 = (StringMirror *)r[1]; 
		StringMirror *sc2 = (StringMirror *)r[2]; 
		size1 = sc1->count_ * 16;
		size2 = sc2->count_ * 16;
		VG_(strcpy)(content1, get_stringobj_str(sc1));
		VG_(strcpy)(content2, get_stringobj_str(sc2));
		EXP_LOGI("%s  %s\n", content1, content2);
		isTainted1 = memory_is_tainted((Addr)sc1->value_, size1);
		isTainted2 = memory_is_tainted((Addr)sc2->value_, size2);
		if((VG_(strcmp)("+8618511308614", content1) == 0) || (VG_(strcmp)("18511308614", content1) == 0)) {
			r[0] = 1;
			ret = r[14];
			return ret;
		}
		if( isTainted1 || isTainted2 ) {
			r[0] = cmp_string_value(sc1, sc2);
			ret = r[14];
			if(isTainted1) {
				size = size2;
				get_memory_dep((Addr)sc1->value_, size, dep1, 0 );
				VG_(snprintf)(dep, DEP_MAX_LEN, "CmpEQ%d(%s,STR:%d(%s))", size, dep1, size, content2);
			} else {
				size = size1;
				get_memory_dep((Addr)sc2->value_, size, dep2, 0 );
				VG_(snprintf)(dep, DEP_MAX_LEN, "CmpEQ%d(STR:%d(%s),%s)", size, size, content1, dep2);
			}
			flip_register(8, 1);
			update_register_dep(8, 32, dep);
			EXP_LOGI("hook_string_equals(), return ret=0x%08x, dst=0x%08x, res=%d\nstr1(0x%08x): %s\nstr2(0x%08x): %s\n", 
					ret, dst, r[0], (Addr)((StringMirror *)r[1])->value_, content1, 
					(Addr)((StringMirror *)r[2])->value_, content2);
		//r[15] = r[14];
		} else {
			return ret;
		}
