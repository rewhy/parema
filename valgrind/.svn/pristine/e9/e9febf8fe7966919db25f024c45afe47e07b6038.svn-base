...
#else
static VG_REGPARM(0) void helperc_binop(IRStmt *clone, UInt tmp1_value, UInt tmp2_value, UInt end_size) {
	UInt j1 = 0, j2 = 0;
	Bool b1 = False, b2 = False;
	char *p;
	char buffer[XXX_MAX_BUF];
	...
	Tmp tmp1 = (e1->tag == Iex_RdTmp) ? e1->Iex.RdTmp.tmp : INVALID_TMP;
	Tmp tmp2 = (e2->tag == Iex_RdTmp) ? e2->Iex.RdTmp.tmp : INVALID_TMP;
	Tmp tmp_to = clone->Ist.WrTmp.tmp;
		...
		if (b1 || b2) {
			...
			switch (op) {
				case Iop_Shl8 ... Iop_Sar64:
					size = 8;
					break;
				case Iop_HAdd8Ux4 ... Iop_CmpNEZ8x4:
					size = 32;
					break;
				default:
					switch (*p) {
...
#else
//static VG_REGPARM(0) void helperc_x86g_calculate_condition(UInt cond, UInt cc_op, Tmp cc_dep1, Tmp cc_dep2,
//		UInt cc_dep1_value, UInt cc_dep2_value) {
//}
// For ARM, the result also depends on cc_dep3 (TBD)
//static VG_REGPARM(3) void helperc_armg_calculate_condition(IRStmt *clone, UInt cond_n_op_value, UInt cc_dep1_value, UInt cc_dep2_value, UInt cc_dep3_value) {
static VG_REGPARM(0) void helperc_armg_calculate_condition(IRStmt *clone, UInt cond_n_op_value, UInt cc_dep1_value, UInt cc_dep2_value) {
	UInt j1 = 0, j2 = 0;
	Bool b1 = False, b2 = False;
	char type = 'I';
	int size = 32;
	...
	UInt tmp_to = clone->Ist.WrTmp.tmp;
	//UInt cond = args[0]->Iex.Const.con->Ico.U32;
	UInt cond  = cond_n_op_value >> 4;
	//UInt cc_op = args[1];
	UInt cc_op = cond_n_op_value & 0xF;
	Tmp  cc_n_op = args[0]->Iex.RdTmp.tmp; 
	Tmp  cc_dep1 = args[1]->Iex.RdTmp.tmp;
	Tmp  cc_dep2 = args[2]->Iex.RdTmp.tmp;
	//Tmp  cc_dep3 = args[3]->Iex.RdTmp.tmp;
	...
	if (b1 || b2) {        
		if (b1 && b2) {
			SPRINTF_CONS(deptmp[j2], "armg_calculate_condition(0x%x:I32,0x%x:I32,%s,%s)", cond, cc_op, deptmp[cc_dep1].cons, deptmp[cc_dep2].cons);
			ppDepTmp(&deptmp[j2]);
		}
		else if (b1) {
			SPRINTF_CONS(deptmp[j1], "armg_calculate_condition(0x%x:I32,0x%x:I32,%s,0x%x:%c%d)", cond, cc_op, deptmp[cc_dep1].cons, cc_dep2_value, type, size);
			ppDepTmp(&deptmp[j1]);
		}
		else if (b2) {
			SPRINTF_CONS(deptmp[j2], "armg_calculate_condition(0x%x:I32,0x%x:I32,0x%x:%c%d,%s)", cond, cc_op, cc_dep1_value, type, size, deptmp[cc_dep2].cons);
			ppDepTmp(&deptmp[j2]);
		}
		return;
	} 
				...
				switch (st->Ist.WrTmp.data->tag) {
					...
					case Iex_Binop:
...
#else
							case Iop_AddF64 ... Iop_F32toF16:
							case Iop_I32UtoFx2 ... Iop_Abs32Fx2:
							case Iop_AddD64 ... Iop_Sqrt64F0x2:
#endif
								j = 1;
								break;
							default:
						...
						if (j == 1) {
							...
							add_dirty7(helperc_binop,
									...
									mkIRExpr_HWord(0));
#else
							stclone = deepMallocIRStmt(st);
							add_dirty4(helperc_binop,
									mkIRExpr_HWord((HWord)stclone),
									mkIRExpr_HWord(0),
									mkIRExpr_HWord(0),
									mkIRExpr_HWord(0));
#endif
					...
					case Iex_CCall:
						...
						if (VG_(strcmp)(st->Ist.WrTmp.data->Iex.CCall.cee->name, "x86g_calculate_condition") == 0) {
							...
							//tl_assert(args[0] >= X86CondO && args[0] <= X86CondAlways);
							//tl_assert(args[1]->tag == Iex_RdTmp);
							if (args[1]->tag == Iex_RdTmp) { // cc_op
							...
							if (VG_(strcmp)(st->Ist.WrTmp.data->Iex.CCall.cee->name, "armg_calculate_condition") == 0) {
								args = st->Ist.WrTmp.data->Iex.CCall.args;
								//tl_assert(cond >= ARMCondEQ && cond <= ARMCondNV);
								//tl_assert(args[0]->tag == Iex_Const && args[0]->Iex.Const.con->tag == Ico_U32);
								//tl_assert(args[1]->tag == Iex_RdTmp);
								if (args[0]->tag == Iex_RdTmp) {
									tl_assert(args[1]->tag == Iex_RdTmp);
									tl_assert(args[2]->tag == Iex_RdTmp);
									tl_assert(args[3]->tag == Iex_RdTmp);

									stclone = deepMallocIRStmt(st);
									add_dirty4(helperc_armg_calculate_condition,
											mkIRExpr_HWord((HWord)stclone),
											args[0],
											args[1],
											args[2]);
									...
									UInt cond  = args[0]->Iex.Const.con->Ico.U32 >> 4;
									UInt cc_op = args[0]->Iex.Const.con->Ico.U32 & 0xF;
									//VG_(printf)("oops, we depend of armg_calculate_condition: %d, %d\n", cond, cc_op);
									add_dirty2(helperc_rdtmp,
											mkIRExpr_HWord(INVALID_TMP),
											mkIRExpr_HWord(st->Ist.WrTmp.tmp));
								}
								//VG_(tool_panic)("Iex_CCall");
#endif
							}
							else {
								// just remove the dependency
