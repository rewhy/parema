...
STRNLEN(VG_Z_LIBC_SONAME, strnlen)
	...
	//DO_CREQ_v_WW(VG_USERREQ__WRAPPER_REP_STRLEN, const char*, str, SizeT, i);
...
{ \
	SizeT i = 0; \
	while (str[i] != 0) i++; \
	DO_CREQ_v_WW(VG_USERREQ__WRAPPER_REP_STRLEN, void*, str, int, i); \
	return i; \
}
...
{ \
	...
	*dst = 0; \
	...
	\
	DO_CREQ_v_WW(VG_USERREQ__WRAPPER_REP_STRCPY, void*, src_orig, void*, dst_orig); \
	return dst_orig; \
}
...
{ \
	SizeT n = 0; \
	DO_CREQ_v_WWW(VG_USERREQ__WRAPPER_REP_STRNCMP, char*, s1, char*, s2, int, nmax); \
	while (True) { \
		if (n >= nmax) break; \
		if (*s1 == 0 && *s2 == 0) break; \
		if (*s1 == 0) return -1; \
...
{ \
	...
	register UChar c1; \
	register UChar c2; \
	DO_CREQ_v_WW(VG_USERREQ__WRAPPER_REP_STRCASECMP, char*, s1, char*, s2); \
	while (True) { \
...
{ \
	extern int tolower(int); \
	SizeT n = 0; \
	DO_CREQ_v_WWW(VG_USERREQ__WRAPPER_REP_STRNCASECMP, char*, s1, char*, s2, UInt, nmax); \
	while (True) { \
		if (n >= nmax) return 0; \
...
{ \
	register UChar c1; \
	register UChar c2; \
	DO_CREQ_v_WW(VG_USERREQ__WRAPPER_REP_STRCMP, char*, s1, char*, s2); \
	while (True) { \
		c1 = *(const UChar *)s1; \
			c2 = *(const UChar *)s2; \
			if (c1 != c2) break; \
...
{ \
	...
	\
	if (len > 0) { \
		DO_CREQ_v_WWW(VG_USERREQ__WRAPPER_REP_MEMMOVE_OR_MEMCPY, char*, dst, char*, src, UInt, len); \
...
{ \
	...
	Addr s2A = (Addr)s2V; \
	\
	DO_CREQ_v_WWW(VG_USERREQ__WRAPPER_REP_MEMCMP, char*, s1V, char*, s2V, UInt, n); \
	if (((s1A | s2A) & WM) == 0) { \
...
#define MEMSET(soname, fnname) \
	...
	(void *s, Int c, SizeT n) \
{ \
	DO_CREQ_v_WWW(VG_USERREQ__WRAPPER_REP_MEMSET, char*, s, Int, c, SizeT, n); \
	if (sizeof(void*) == 8) { \
		Addr  a  = (Addr)s;   \
		ULong c8 = (c & 0xFF); \
		c8 = (c8 << 8) | c8; \
...
{ \
	...
	\
	DO_CREQ_v_WW(VG_USERREQ__WRAPPER_REP_STRSTR, char*, h, char*, n); \
