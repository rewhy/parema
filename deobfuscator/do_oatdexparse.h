...
#if DBG_OAT_PARSE
#define	OAT_LOGI(fmt, x...) if(is_parse_oat) VG_(printf)(fmt, ##x)
...
// 32 bytes
struct DexClassDef {
	/*0x00*/ UShort  classIdx;           /* index into typeIds for this class */
					 UShort	 pad1;
	/*0x04*/ UInt    accessFlags;
	/*0x08*/ UShort  superclassIdx;      /* index into typeIds for superclass */
					 UShort	 pad2;
...
INLINE const struct DexTypeId* dexGetTypeId(const struct DexFile* pDexFile, UInt idx) {
	if(idx >= pDexFile->pHeader->typeIdsSize) {
		VG_(printf)("Error: idx=%d typeIdsSize=%d\n", idx, pDexFile->pHeader->typeIdsSize);
		tl_assert(0);
	}
